...
const BUNDESLAENDER: Record<string, { name: string; rate: number; info: string }> = {
  ...
  'nordrhein-westfalen': { name: 'Nordrhein-Westfalen', rate: 6.5, info: 'NRW hat mit 6,5% einen der höchsten Sätze in Deutschland.' },
  ...
  'mecklenburg-vorpommern': { name: 'Mecklenburg-Vorpommern', rate: 6.0, info: 'Mecklenburg-Vorpommern erhebt 6,0%.' },
  ...
  'saarland': { name: 'Saarland', rate: 6.5, info: 'Das Saarland hat 6,5%.' },
  ...
  'thueringen': { name: 'Thüringen', rate: 6.5, info: 'Thüringen erhebt 6,5% Grunderwerbsteuer.' },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://immo-rechner.net/grunderwerbsteuer/${bundesland}` },
  };
}

export default async function GrunderwerbsteuerPage({ params }: Props) {
  const { bundesland } = await params;
  const bl = BUNDESLAENDER[bundesland];
  if (!bl) notFound();
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="prose prose-lg max-w-none">
          ...
          <p>
            ...
            dem Saarland, Schleswig-Holstein und Thüringen.
          </p>
